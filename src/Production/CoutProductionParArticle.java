...
public class CoutProductionParArticle extends JLayeredPane implements Constantes{
	 ...
	 */
	public CoutProductionParArticle() {
		setOpaque(true);
...
*/
		  ...
		  button.setBounds(786, 760, 104, 31);
  	...
  	for(int s=0;s<listEtatCoutProduction.size();s++)
  	{
		...
		for(int n=0;n<listProductions.size();n++)
		{
			...
			for(int k=0;k<listCoutMP.size();k++)
			{
				...
				coutTotalDechetMP=coutTotalDechetMP.add(listCoutMP.get(k).getQuantDechet().multiply(listCoutMP.get(k).getMatierePremier().getPrix()));
				if(!listCoutMP.get(k).getMatierePremier().getCategorieMp().getSubCategorieMp().getCode().equals(Constantes.SOUS_CATEGORIE_MATIERE_PREMIERE_CADEAU))
				{
					...
					coutTotalOffreMP=coutTotalOffreMP.add(listCoutMP.get(k).getQuantiteOffre().multiply(listCoutMP.get(k).getMatierePremier().getPrix()));
				}
				...
				coutTotalCoutQuantiteConsommeMP=coutTotalCoutQuantiteConsommeMP.add(quantiteConsommeSoustraireQuantitePlus.multiply(listCoutMP.get(k).getMatierePremier().getPrix()));
				coutTotalDechetFournisseurMP=coutTotalDechetFournisseurMP.add(listCoutMP.get(k).getQuantDechetFournisseur().multiply(listCoutMP.get(k).getMatierePremier().getPrix()));
				coutTotalManquanteMP=coutTotalManquanteMP.add(listCoutMP.get(k).getQuantiteManquante().multiply(listCoutMP.get(k).getMatierePremier().getPrix()));
		...
		etatCoutProduction.setCoutEmployeGenerique(coutTotalEmployeGenerique.add(coutTotalSupp25EmployeGenerique).add(coutTotalSupp50EmployeGenerique));
		etatCoutProduction.setCoutEmployeProduction (coutTotalEmployeProduction.add(coutTotalSupp25EmployeProduction.add(coutTotalSupp50EmployeProduction)));
		etatCoutProduction.setCoutEmployeEmballage(coutTotalEmployeEmballage.add(coutTotalSupp25EmployeEmballage).add(coutTotalSupp50EmployeEmballage));
		etatCoutProduction.setQuantiteRealiser(QuantiteReel);
	...
	dfDecimal.setGroupingUsed(true);
		...
		{	
			...
			TotalRealiser=TotalRealiser.add(etatCoutProduction.getQuantiteRealiser());
					 ...
					 Object []ligne={etatCoutProduction.getArticle().getCodeArticle(),etatCoutProduction.getArticle().getLiblle(),dfDecimal.format(etatCoutProduction.getCoutMP().setScale(2, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getCoutEmployeGenerique().setScale(6, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getCoutEmployeProduction().setScale(6, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getCoutEmployeEmballage().setScale(2, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getTotal().setScale(6, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getQuantiteRealiser().setScale(6, RoundingMode.HALF_UP)),dfDecimal.format(etatCoutProduction.getCout().setScale(2, RoundingMode.HALF_UP))};
